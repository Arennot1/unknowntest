...
// drop the matching file in assets/clients/.
const CLIENT_LOGOS = [
...
function renderSectionHTML(project, key, label) {
    ...
    if (s.type === 'rich') {
        return `<section id="${id}" class="scroll-mt-32">
            <h2 class="text-2xl font-bold uppercase tracking-tight mb-8 border-b border-gray-200 pb-4">${label}</h2>
            <div class="case-study-rich text-gray-700">
                ${s.content}
            </div>
        </section>`;
    }
            ...
            <div class="max-w-[1400px] mx-auto px-4 md:px-8 lg:px-10">
                ...
                <div class="journey-block">
                    ...
                    <div class="journey-trusted">
                        ...
                        <!-- Client logos are driven by CLIENT_LOGOS above — set an entry's 'image'

                             to a real file path (e.g. 'assets/clients/nimbus-labs.png') to swap that

                             chip from a bracketed placeholder to the actual logo; no name text is

                             shown either way. The visible strip and its seamless-loop duplicate are

                             both generated from this one list, so they can no longer drift apart. -->
